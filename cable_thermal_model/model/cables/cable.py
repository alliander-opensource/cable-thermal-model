...
class Cable(AbstractCable):
    ...
    def __init__(
        ...
        self.radii_grid = np.array([], dtype=float)
        self.grid_deltas = np.array([], dtype=float)
        self.layer_name_grid = np.array([], dtype=object)
        self.rho_grid = np.array([], dtype=float)
        self.capacity_grid = np.array([], dtype=float)
        self.alpha_grid = np.array([], dtype=float)
        self.surface_area_grid = np.array([], dtype=float)
...
class CableTrefoilCircuitSinglePipeInSoil(CableSoil):
    ...
    def integrate_timestep(
        ...
        """Solve the heat equation using implicit Euler finite-difference method.

        For internal heating mode, an additional heat source is added between the
        pipe and equivalent trefoil cable. This source represents twice the cable
        sheath heat loss (equivalent to three cables in trefoil). The banded matrix
        is converted to sparse format to accommodate this source.
        ...
        A_dia = sparse.dia_matrix((A_banded, [1, 0, -1]), shape=(A_banded.shape[1], A_banded.shape[1]))
        A_sparse = sparse.lil_matrix(A_dia)
...
class CableTrefoilCircuitSinglePipeInAir(CableTrefoilCircuitSinglePipeInSoil, CableAir):
    ...
    def integrate_timestep(
        ...
        """Solve the heat equation for cables in air using implicit Euler method.
